...
<div class= "bg bg-gray-100 flex items-center justify-center h-screen">
    ...
    <div  class= "bg-white p-8 rounded shadow-md w-96">
        <h1 class="text-2xl font-semibold mb-4 text-center">User Set Password</h1>
        @error('user')
        ...
        <form action ="/user-set-forgot-password" method = "post" class="space-y-4  "   >
            ...
            <div>
                ...
                <input type="hidden" id="email" name="email" placeholder=" Enter User email" value={{$email}} class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm    focus:ring-indigo-500 focus:border-indigo-500 ">
                @error('email')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
        ...
        <div class="mt-4">
                ...
                <button type="submit" class="w-full bg-indigo-500 rounded-2xl px-4 py-2 text-white hover:bg-indigo-500">Update Password</button>
         </div>
        </form >      
